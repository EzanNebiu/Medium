...
export default function AdminOrders() {
  ...
  const changeStatus = async (orderId: string, nextStatus: OrderStatus) => {
    setOrders((current) => current.map((order) => String(order.id) === orderId ? { ...order, status: nextStatus } : order));
    const result = await updateOrderStatus(orderId, nextStatus);
    if (result.error) {
      showToast("Statusi nuk mund të përditësohej.", "error");
      return;
    }
    showToast(nextStatus === "shipped"
      ? result.emailSent
        ? "Statusi u ndryshua dhe emaili i dërgesës u nis."
        : "Statusi u ndryshua. Emaili nuk u nis sepse provider-i nuk është konfiguruar."
      ...
      <Card className="mt-6 p-4">
        ...
        <div className="mt-5 overflow-auto">
          <table className="w-full min-w-[920px] text-left text-sm">
            ...
            <tbody className="divide-y">{filtered.map((order) => <tr key={String(order.id)}><td className="py-3 font-bold">{String(order.id).slice(0, 8)}</td><td>{String(order.customer_name ?? "")}<p className="text-xs text-muted-foreground">{String(order.customer_email ?? "")}</p></td><td><Select value={String(order.status)} onChange={(event) => void changeStatus(String(order.id), event.target.value as OrderStatus)}><option value="pending">në pritje</option><option value="confirmed">konfirmuar</option><option value="processing">duke u përgatitur</option><option value="shipped">dërguar</option><option value="delivered">dorëzuar</option><option value="cancelled">anuluar</option></Select></td><td>{formatCurrency(Number(order.total ?? 0))}</td><td>{sqPayment(String(order.payment_method ?? ""))}{order.payment_method === "monthly" ? <p className="text-xs text-muted-foreground">{String(order.installment_months ?? "")} muaj</p> : null}</td><td>{String(order.payment_status ?? order.payment_provider ?? "unpaid")}</td><td>{sqDeliveryMethod(String(order.delivery_method ?? "delivery"))}</td><td>{String(order.city ?? "")}</td></tr>)}</tbody>
          </table>
